...
import { FaRedo, FaStepBackward, FaStepForward, FaPlay, FaPause, FaRandom } from "react-icons/fa";
  ...
  return (
    <div className={`${HeaderCSS.App}`}>
      <section className={HeaderCSS.music__container}>
        ...
        <div className={HeaderCSS.music__cd} >
          <animated.div style={{...styles}}>
            <img id="cd" className={HeaderCSS.music__cd__thumb} src={songs[currentIndex].image} alt="" ref = {cd}/>
          </animated.div>
        </div>
        
        <div className={HeaderCSS.music__controllers__btn}>
          <div className={ isRedo ? `${HeaderCSS.music__redo__btn}  ${HeaderCSS.active}` :  `${HeaderCSS.music__redo__btn}`} onClick={ audioRedo }>
            <FaRedo/>
          </div>
          <div className={HeaderCSS.music__prev__btn} onClick={prevSong}>
            <FaStepBackward />
          </div>
          <div className={HeaderCSS.music__toggle__play__btn} onClick={ audioPlay }>
            {isPlay ? <FaPlay/> :<FaPause/>}
          </div>
          <div className={HeaderCSS.music__next__btn} onClick={nextSong}>
            <FaStepForward/>
          </div>
          <div className={ isRandom ? `${HeaderCSS.music__random__btn}  ${HeaderCSS.active}` :  `${HeaderCSS.music__random__btn}`} onClick={ audioRandom}>
            <FaRandom/>
          </div>
        </div>
        <input id="progress" className={HeaderCSS.music__progress} type="range" defaultValue="0" step="1" min="0" max="100" onChange={audioSeek}  />
        <audio id="audio" src={songs[currentIndex].path} onTimeUpdate={onTimeUpDate} onEnded={nextSong}/>
      </section>

      <section className={HeaderCSS.music__playlist}>
        {songs.map((song,index) =>{
          return <MusicItem  key = {index}
       {...song} currentIndex={currentIndex} songIndex={index} 
